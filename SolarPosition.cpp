...
SolarPosition_t calculateSolarPosition(time_t tParam, float Latitude, float Longitude)
{
	...
	// TODO: Check if we need utc_tm
	tm utc_tm = *gmtime(&tParam);
	auto year = utc_tm.tm_year + 1970;
	...
	if (tParam != timePrevious || Latitude != latPrevious || Longitude != lonPrevious) // only calculate if time or location has changed
	{
		...
		sunCenter = DEG_TO_RAD * ((1.9146 - 0.004847 * elapsedT) * sin(solarMeanAnomaly) + (0.019993 - 0.000101 * elapsedT) * sin(2 * solarMeanAnomaly) + 0.00029 * sin(3 * solarMeanAnomaly));

		solarTrueAnomaly = solarMeanAnomaly + sunCenter;
		equatorObliquity = DEG_TO_RAD * (23 + 26 / 60. + 21.448 / 3600. - 46.815 / 3600 * elapsedT);

		JDx = JD_whole - Y2K_JULIAN_DAY;

		GreenwichHourAngle = 280.46061837 + (360 * JDx) % 360 + .98564736629 * JDx + 360.98564736629 * JD_frac;
		...
		// results:
		result.distance = 1.000001018 * (1 - earthOrbitEccentricity * earthOrbitEccentricity) / (1 + earthOrbitEccentricity * cos(solarTrueAnomaly));
		...
		result.azimuth = PI + atan2(sin(hourAngle),
